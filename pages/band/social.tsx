...
const CREDENTIAL_FIELDS: Record<Platform, { key: string; label: string; placeholder: string; secret?: boolean }[]> = {
  discord:   [{ key: 'webhook_url', label: 'Webhook URL', placeholder: 'https://discord.com/api/webhooks/...', secret: true },
               { key: 'server_id',  label: 'Server ID (optional)', placeholder: '1234567890' }],
  facebook:  [{ key: 'page_id',           label: 'Page ID', placeholder: '123456789012345' },
               ...
               { key: 'page_handle',       label: 'Page Handle', placeholder: 'YourBandPage' }],
...
export default function BandSocial() {
  ...
  const loadAccounts = async (actId?: string) => {
    const id = actId || myAct?.id;
    if (!id) return;
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) return;
    const res = await fetch(`/api/social/accounts?actId=${id}`, {
  ...
  const updatePost = async (id: string, status: SocialStatus) => {
    setSaving(id);
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) return;
  ...
  const saveCredentials = async () => {
    ...
    const res = await fetch('/api/social/accounts', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` },
      body: JSON.stringify({ actId: myAct.id, platform: connectPlatform, credentials, handle }),
    });
    setCredSaving(false);
    if (res.ok) {
      setCredSaved(true);
  ...
  const disconnectPlatform = async (platform: Platform) => {
    ...
    await fetch('/api/social/accounts', {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` },
      body: JSON.stringify({ actId: myAct.id, platform }),
    });
    await loadAccounts();
  };
    ...
    <AppShell requireRole="act_admin">
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">{myAct?.act_name || 'Your band'} · AI-drafted show announcements</div>
        </div>
      </div>
              ...
              <div key={post.id} className="card">
                ...
                <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
                  {post.status === 'pending' && editing !== post.id && (
                    <>
                      <button className="btn btn-secondary btn-sm" disabled={saving === post.id} onClick={() => updatePost(post.id, 'approved')}>
                        {saving === post.id ? 'Approving…' : '✓ Approve'}
                      </button>
                      <button className="btn btn-ghost btn-sm" style={{ color: '#f87171' }} disabled={saving === post.id} onClick={() => updatePost(post.id, 'dismissed')}>
                        Dismiss
                      </button>
                    </>
                  )}
                  ...
                  {post.status === 'approved' && (
                    <>
                      ...
                      <button className="btn btn-ghost btn-sm" onClick={() => updatePost(post.id, 'posted')} disabled={saving === post.id}>
                        Mark Posted
                      </button>
                    </>
                  )}
                  ...
                  {post.status === 'dismissed' && (
                    <button className="btn btn-ghost btn-sm" disabled={saving === post.id} onClick={() => updatePost(post.id, 'pending')}>
                      Restore
                    </button>
                  )}
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                  <div style={{ fontFamily: 'var(--font-body)', fontSize: '0.82rem', color: 'var(--text-muted)', lineHeight: 1.55 }}>
                    {p.autoPost
                      ...
                      : `${p.label} doesn't support text-only auto-posting. Save your handle and "Copy + Open ${p.label}" will copy the post and open your profile.`}
                  </div>
