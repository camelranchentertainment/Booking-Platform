        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {selected && selectedShows.length > 0 && (
            <div className="card">
              ...
              {selectedShows.map((s: any) => (
                <Link key={s.id} href={`/bookings/${s.id}`} style={{ display: 'block', textDecoration: 'none', padding: '0.75rem', background: 'var(--bg-overlay)', borderRadius: 'var(--radius-sm)', marginBottom: '0.5rem' }}>
          ...
          <div className="card">
            ...
            {shows.filter(s => s.show_date >= todayStr).slice(0, 8).map((s: any) => (
              <Link key={s.id} href={`/bookings/${s.id}`} style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', padding: '0.5rem 0', borderBottom: '1px solid var(--border)', textDecoration: 'none' }}>
