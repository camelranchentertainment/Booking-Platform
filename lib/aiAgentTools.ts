...
export const STAGE_PAYMENT_SETTLE_TOOL = {
  ...
  description:
    "Propose updating a booking's payment status (amount received, and/or " +
    "marking it pending/received/settled). Does NOT write to the database — " +
  ...
  input_schema: {
    ...
    properties: {
      ...
      actual_amount_received: { type: 'number' },
      payment_status: {
        type: 'string',
...
export async function execStagePaymentSettle(
  actId: string,
  userId: string,
  args: { booking_id: string; actual_amount_received?: number; payment_status?: string }
) {
  if (args.actual_amount_received === undefined && args.payment_status === undefined) {
    throw new Error('Nothing to change — specify actual_amount_received and/or payment_status.');
  }

  const { data: current, error: curErr } = await supabase
    .from('bookings')
    .select('id, actual_amount_received, payment_status, show_date, venue:venues(name)')
    .eq('id', args.booking_id)
    .eq('act_id', actId)
    .maybeSingle();
  ...
  // Explicit whitelist — only these two fields, ever.
  const payload = {
    booking_id: args.booking_id,
    actual_amount_received: args.actual_amount_received !== undefined
      ? args.actual_amount_received : current.actual_amount_received,
    payment_status: args.payment_status !== undefined
      ? args.payment_status : current.payment_status,
